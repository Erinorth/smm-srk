...
class MRCondition extends Component
{
    public $mRConditionId;
    /**
     * Create a new component instance.
     *
     * @return void
     */
    public function __construct($mRConditionId)
    {
        $this->mRConditionId = $mRConditionId;
    }
                ...
                <div class="container-sm">
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-2">
                        @foreach ($mrcondition as $value)
                            ...
                            <h6>Condition : {{$value->Condition}}</h6>
                        @endforeach
                    </div>
                </div>
            </div>
...
blade;
    ...
    public function mrcondition()
    {
        $mrcondition = DB::select('SELECT jobs.id AS job_id, projects.ProjectName, locations.LocationName, products.ProductName, machines.MachineName, systems.SystemName, equipment.EquipmentName, scopes.ScopeName, jobs.Remark, activities.ActivityName, m_r_conditions.Condition, m_r_conditions.id
            FROM ((((equipment INNER JOIN (products INNER JOIN (systems INNER JOIN item_sets ON systems.id = item_sets.system_id) ON products.id = item_sets.product_id) ON equipment.id = item_sets.equipment_id) INNER JOIN ((machines INNER JOIN (locations INNER JOIN machine_sets ON locations.id = machine_sets.location_id) ON machines.id = machine_sets.machine_id) INNER JOIN ((scopes INNER JOIN (items INNER JOIN jobs ON items.id = jobs.item_id) ON scopes.id = items.scope_id) INNER JOIN projects ON jobs.project_id = projects.id) ON machine_sets.id = items.machine_set_id) ON item_sets.id = items.item_set_id) INNER JOIN activities ON items.id = activities.item_id) INNER JOIN maintenance_reports ON activities.id = maintenance_reports.activity_id) INNER JOIN m_r_conditions ON maintenance_reports.id = m_r_conditions.maintenance_report_id
            WHERE (((m_r_conditions.id)='.$this->mRConditionId.'))');
