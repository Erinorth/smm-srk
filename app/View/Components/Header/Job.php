            ...
            <div class="card-body">
                <div class="container-sm">
                    <div class="row row-cols-1 row-cols-sm-2 row-cols-md-2">
                        @foreach ($jobdetail as $value)
                            <h6>Job ID : {{$value->id}}</h6>
                            <h6>Item ID : {{$value->item_id}}</h6>
...
blade;
    ...
    {
        $jobdetail = DB::select('SELECT projects.ProjectName, locations.LocationName, products.ProductName, machines.MachineName, machine_sets.Remark, systems.SystemName, equipment.EquipmentName, scopes.ScopeName, jobs.Remark, jobs.id, jobs.item_id
