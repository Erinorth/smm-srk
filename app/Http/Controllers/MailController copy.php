...
class MailController extends Controller
{
    public function certificate()
    {
        ...
        $mail->CharSet = 'utf-8';
        $mail->isSMTP();
        $mail->Host = 'mail.egat.co.th';             //  smtp host
        $mail->SMTPAuth = true;
        $mail->Username = '951091';   //  sender username
        $mail->Password = '#6z8bihi6i8pencs';       // sender password
        $mail->SMTPSecure = 'ssl';                  // encryption - ssl/tls
        $mail->Port = 465;                          // port - 587/465
        $mail->setFrom('renew.smm@egat.co.th', 'Smart Maintenance Management');
        ...
        foreach ($recipient as $value) {
            ...
            $mail->Body = view('emails.certificate', compact('mail_measuring_all','mail_man_cer_all'))->render();
            $mail->send();
            $mail->clearAddresses();
        }

        $mail->addAddress('569887@egat.co.th');
        $mail->Body = view('emails.send', compact('subject'))->render();
        $mail->send();
        $mail->clearAddresses();
    ...
    public function facility()
    {
        $dropTempTables = DB::unprepared(DB::raw("
                ...
                DROP TABLE IF EXISTS mail_facility_am ;
                DROP TABLE IF EXISTS mail_facility_responsible ;
                DROP TABLE IF EXISTS mail_facility_all ;
                DROP TABLE IF EXISTS crane ;
                DROP TABLE IF EXISTS mail_crane_am ;
                DROP TABLE IF EXISTS mail_crane_responsible ;
                DROP TABLE IF EXISTS mail_crane_all ;
                DROP TABLE IF EXISTS all_recipient ;
            ")
        );
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.facility', compact('mail_facility_all','mail_crane_all'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    public function finishproject()
    {
        $dropTempTables = DB::unprepared(
            DB::raw("
                DROP TABLE IF EXISTS all_NC_pmorder ;
                DROP TABLE IF EXISTS close_last_week ;
                DROP TABLE IF EXISTS mail_planner ;
                DROP TABLE IF EXISTS mail_planner_close ;
                DROP TABLE IF EXISTS mail_admin ;
                DROP TABLE IF EXISTS mail_admin_close ;
                DROP TABLE IF EXISTS mail_head ;
                DROP TABLE IF EXISTS mail_head_planner ;
                DROP TABLE IF EXISTS mail_head_planner_close ;
                DROP TABLE IF EXISTS mail_all_NC ;
                DROP TABLE IF EXISTS mail_all_C_last_week ;
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.finishproject', compact('near_finish'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    public function milestone()
    {
        $dropTempTables = DB::unprepared(
            DB::raw("
                DROP TABLE IF EXISTS mile_stone_update ;
                DROP TABLE IF EXISTS not_complete_milestone ;
                DROP TABLE IF EXISTS one_time_milestone ;
                DROP TABLE IF EXISTS daily_milestone ;
                DROP TABLE IF EXISTS all_milestone ;
                DROP TABLE IF EXISTS mail_am ;
                DROP TABLE IF EXISTS mail_planer ;
                DROP TABLE IF EXISTS mail_responsible ;
                DROP TABLE IF EXISTS mail_all ;
            ")
        );
        ...
        /* $test = DB::table('mile_stone_update')->get();
        dd($test); */
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.milestone', compact('milestone'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    public function overtime()
    {
        $dropTempTables = DB::unprepared(
            DB::raw("
                DROP TABLE IF EXISTS otnow ;
                DROP TABLE IF EXISTS planot ;
                DROP TABLE IF EXISTS allot ;
                DROP TABLE IF EXISTS order_record ;
                DROP TABLE IF EXISTS running_total_by_group ;
                DROP TABLE IF EXISTS all_data ;
                DROP TABLE IF EXISTS min_this_month ;
                DROP TABLE IF EXISTS min_next_month ;
                DROP TABLE IF EXISTS all_man_x ;
                DROP TABLE IF EXISTS all_man ;
                DROP TABLE IF EXISTS mail_am ;
                DROP TABLE IF EXISTS mail_sup ;
                DROP TABLE IF EXISTS mail_admin ;
                DROP TABLE IF EXISTS mail_head ;
                DROP TABLE IF EXISTS mail_head_am ;
                DROP TABLE IF EXISTS mail_16 ;
                DROP TABLE IF EXISTS mail_head_16 ;
                DROP TABLE IF EXISTS mail_all ;
                DROP TABLE IF EXISTS mail_all_group ;
            ")
        );
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.overtime', compact('noti_this', 'noti_next', 'noti_year'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    public function plan()
    {
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.plan', compact('plan'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    public function pmorder()
    {
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.pmorder', compact('mail_all_NC', 'mail_all_C_last_week'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    /* public function responsible(Request $request)
    {
        ...
        if ($request->ajax()) {
            $data = Email::select('emails.id', 'emails.Responsible', 'project_types.TypeName', 'job_positions.JobPositionName')
                ->join('project_types', 'emails.project_type_id', '=', 'project_types.id')
                ->join('job_positions', 'emails.MilestoneResponsible', '=', 'job_positions.id')
                ->get();
            return DataTables::of($data)
                ...
                ')
                ->rawColumns(['TypeName', 'Responsible', 'id', 'JobPositionName', 'action'])
                ->make(true);
        }

        return view('emails.responsible', compact('type', 'jobposition'));
    }

    public function responsiblestore(Request $request)
    {
        $rules = array(
            'project_type_id' => 'required',
            'Responsible' => 'required',
            'MilestoneResponsible' => 'required'
        );
        ...
        $form_data = array(
            ...
            'MilestoneResponsible' => $request->MilestoneResponsible
        );

        Email::create($form_data);

        return response()->json(['success' => 'Man Hour Added successfully.']);
    }

    public function responsibleedit($id)
    {
        if (request()->ajax()) {
    ...
    public function responsibleupdate(Request $request, Email $id)
    {
        $rules = array(
            'project_type_id' => 'required',
            'Responsible' => 'required',
        ...
        Email::whereId($request->hidden_id)->update($form_data);

        return response()->json(['success' => 'Man Hour is successfully updated']);
    }

    public function responsibledestroy(Request $request)
    {
        $data = Email::findOrFail($id);
        $data->delete();
    } */
    ...
    {
        $dropTempTables = DB::unprepared(DB::raw("
                DROP TABLE IF EXISTS count_man ;
                DROP TABLE IF EXISTS request_man ;
                DROP TABLE IF EXISTS mail_am ;
                DROP TABLE IF EXISTS mail_responsible ;
                DROP TABLE IF EXISTS mail_head ;
                DROP TABLE IF EXISTS mail_head_am ;
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.request_man', compact('request_man'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    {
        $dropTempTables = DB::unprepared(DB::raw("
                DROP TABLE IF EXISTS max_calibrate ;
                DROP TABLE IF EXISTS max_tool_update ;
                DROP TABLE IF EXISTS tool_calibrate ;
                DROP TABLE IF EXISTS mail_head ;
                DROP TABLE IF EXISTS mail_head_responsible ;
                DROP TABLE IF EXISTS mail_all ;
                DROP TABLE IF EXISTS mail_all_group ;
            ")
        );
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.tool_calibrate', compact('calibrate'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
    ...
    {
        $dropTempTables = DB::unprepared(DB::raw("
                DROP TABLE IF EXISTS max_tool_pm ;
                DROP TABLE IF EXISTS max_tool_update ;
                DROP TABLE IF EXISTS tool_pm ;
                DROP TABLE IF EXISTS mail_head ;
                DROP TABLE IF EXISTS mail_head_responsible ;
                DROP TABLE IF EXISTS mail_all ;
                DROP TABLE IF EXISTS mail_all_group ;
        ...
        foreach ($recipient as $value) {
            ...
            $mail->addAddress($value);
            $mail->Body = view('emails.tool_pm', compact('pm'))->render();
            $mail->send();
            $mail->clearAddresses();
        }
