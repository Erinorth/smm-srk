<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\Location;
use DB;
use DataTables;
use Validator;

class LocationController extends Controller
{
    public function index(Request $request)
    {
        if($request->ajax())
        {
            $data = DB::table('locations')->get();
            return DataTables::of($data)
                    ->editColumn('id', function($data) {
                        return '<div class="text-center">'.$data->id.'</div>';
                    })
                    ->editColumn('LocationKKS', function($data) {
                        return '<div class="text-center">'.$data->LocationKKS.'</div>';
                    })
                    ->editColumn('LocationName', function($data) {
                        return '<div class="text-center">'.$data->LocationName.'</div>';
                    })
                    ->editColumn('LocationThaiName', function($data) {
                        return '<div class="text-center">'.$data->LocationThaiName.'</div>';
                    })
                    ->editColumn('Allowance', function($data) {
                        return '<div class="text-center">'.$data->Allowance.'</div>';
                    })
                    ->addColumn('action','
                        <div class="text-center">
                            @role('."'admin|head_engineering'".')
                                <button class="edit_location btn btn-xs btn-default text-warning mx-1 shadow" name="edit" id="{{$id}}" title="Edit"><i class="fa fa-lg fa-fw fa-pen"></i></button>
                                <button class="delete_location btn btn-xs btn-default text-danger mx-1 shadow" name="edit" id="{{$id}}" title="Delete"><i class="fa fa-lg fa-fw fa-trash"></i></button>
                            @else <div class="text-center">N/A</div>
                            @endrole
                        </div>
                    ')
                    ->rawColumns(['id','LocationKKS','LocationName','LocationThaiName','Allowance','action'])
                    ->make(true);
        }
    }

    public function store(Request $request)
    {
        $rules = array(
            'LocationName'=>'required',
            'Allowance'=>'required'
        );

        $error = Validator::make($request->all(), $rules);

        if($error->fails())
        {
            return response()->json(['errors' => $error->errors()->all()]);
        }

        $form_data = array(
            'LocationKKS' => $request->LocationKKS,
            'LocationName' => $request->LocationName,
            'LocationThaiName' => $request->LocationThaiName,
            'Allowance' => $request->Allowance
        );

        Location::create($form_data);

        return response()->json(['success' => 'Data Added successfully.']);
    }

    public function edit($id)
    {
        if(request()->ajax())
        {
            $data = Location::findOrFail($id);
            return response()->json(['result' => $data]);
        }
    }

    public function update(Request $request, Location $location)
    {
        $rules = array(
            'LocationName'=>'required',
            'Allowance'=>'required'
        );

        $error = Validator::make($request->all(), $rules);

        if($error->fails())
        {
            return response()->json(['errors' => $error->errors()->all()]);
        }

        $form_data = array(
            'LocationKKS' => $request->LocationKKS,
            'LocationName' => $request->LocationName,
            'LocationThaiName' => $request->LocationThaiName,
            'Allowance' => $request->Allowance
        );

        Location::whereId($request->hidden_id_location)->update($form_data);

        return response()->json(['success' => 'Data is successfully updated']);
    }

    public function destroy($id)
    {
        $data = Location::findOrFail($id);
        $data->delete();
    }
}
