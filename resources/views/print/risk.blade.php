...
@section('content')
    ...
    งาน @foreach ($job as $value){{$value->ProjectName}} // {{$value->LocationName}} // {{$value->LocationName}} // {{$value->MachineName}} // {{$value->Remark}} // {{$value->SystemName}} // {{$value->EquipmentName}} // {{$value->ScopeName}} @endforeach
        ...
        <table class="table table-bordered table-sm">
            ...
            <tbody>
                ...
                @foreach ($risk as $value)
                    <tr>
                        @if ( $value->ActivityName <> $activity )
                            <td rowspan="{{$value->CountOfActivityName}}" class="text-center"> {{$i}} </td>
                            <td rowspan="{{$value->CountOfActivityName}}"> {{$value->ActivityName }} </td>
                            @php
                                $activity = $value->ActivityName;
                                $i++;
                            @endphp
                        @endif
                        @if ( $value->CodeHazardName <> $hazard )
                            <td rowspan="{{$value->CountOfHazardName}}"> {{$value->HazardName }} </td>
                            @php
                                $hazard = $value->CodeHazardName;
                            @endphp
                        @endif
        ...
        <table class="table table-bordered table-sm">
            <tbody>
                <tr>
                    ...
                    <td class="text-center" style="width:33%">Rev.2</td>
                </tr>
            </tbody>
        </table>
    </div>
