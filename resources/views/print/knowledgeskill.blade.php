...
@section('title,Knowledge-Skilled')
...
@section('content')
    <div class="text-center">
        <h2>ใบบันทึกรายการความรู้ และทักษะในงานของผู้ปฏิบัติงาน</h2>
    </div>
    ...
    <div class="table-responsive">
        <table class="table table-striped table-bordered table-sm">
            ...
            <tbody>
                @foreach ($knowledgeskill as $value)
                    <tr>
                        <td class="text-center">{{$i}}</td>
                        <td class="text-center">{{$value->JobPosition}}</td>
                        <td>{{$value->TypeofJob}}</td>
                        <td class="text-center">{{$value->KnowledgeSkilled}}</td>
