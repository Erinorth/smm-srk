...
@section('content')
        ...
        <div class="container">
            <div class="row align-items-center justify-content-around">
                <div class="col-xl-2 text-center">
                    <form class="form-horizontal" method="POST" action="{{ url('/update_product_stakeholder_expectation') }}">
                        @csrf
                        <div class="row">
                ...
                <div class="col-xl-2 text-center">
                    <a href="{{ url('product/'.$departmentid) }}" class="btnprn btn btn-info btn-block btn-sm">Print</a>
                </div>
                <div class="col-xl-8 text-center">
                    <a href="{{ url('QSHs_product_expectation_factor/'.$departmentid) }}" class="btn btn-info btn-block btn-sm">บัญชีระบุปัจจัยที่มีผลต่อคุณภาพ อาชีวอนามัยและความปลอดภัยของผลิตภัณฑ์และบริการ</a>
                </div>
            </div>
        </div>
    </x-card.default-card>

    <x-data-table.default-data-table color="" collapse-card="" title="บัญชี ภารกิจ ผลิตภัณฑ์และบริการ ของหน่วยงาน"
        collapse-button="minus" table-id="">
        <x-slot name="tool">
        </x-slot>
        <th>รหัสผลิตภัณฑ์</th>
        <th>ภารกิจ/ผลิตภัณฑ์และบริการ</th>
        <th>ลูกค้า/ผู้มีส่วนเกี่ยวข้อง</th>
    ...
    <x-content.product-stakeholder/>

    <x-content.stakeholder-expectation/>
@endsection
...
@section('js')
    <script>
        $(document).ready(function(){
            ...
            $('#data_table').DataTable({
                ...
                drawCallback: function(){
                    ...
                    $('.toggle-class').on('switchChange.bootstrapSwitch', function (event, state) {
                        ...
                        var product_stakeholder_expectation_id = $(this).data('id');
                        $.ajax({
                            type: "POST",
                            dataType: "json",
                            url: '/product_stakeholder_expectation_related',
                            headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}' },
                            data: {'Related': Related, 'product_stakeholder_expectation_id': product_stakeholder_expectation_id},
            ...
            <x-j-s.product-stakeholder/>

            <x-j-s.stakeholder-expectation/>
        });
    </script>
@endsection
